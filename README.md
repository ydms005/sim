...
>
> 대학 목록(이름·지역·설립구분)은 실제 대학 기준이지만, **경쟁률·모집인원·지원자 수·모집요강 PDF·자료실 파일·대학소식은
> 화면을 보여 주기 위해 만든 개발용 샘플 데이터**이며 실제 수치가 아닙니다.
> 실제 입시 정보는 반드시 **각 대학 입학처 홈페이지**와 **대입정보포털 어디가(<https://www.adiga.kr>)** 에서 확인하세요.
> 이 사이트의 내용을 학생 상담이나 지원 판단의 근거로 쓰면 안 됩니다.
...
| 모집요강 | `/sim/univ/3/guideline` | 학년도별 모집요강 PDF 보기(확대·축소·다운로드·인쇄) |
...
- `IS_SAMPLE_DATA` 는 **모든** 경쟁률·자료가 실제 자료로 바뀐 뒤에만 `false` 로 바꾸세요.
...
- 현재 포함된 모집요강·자료실 PDF와 대학소식은 모두 샘플이며, 모든 쪽에 "샘플" 표시가 들어 있습니다.
- 실제 대학 자료(모집요강, 전형 결과, 면접 기출 등)를 올릴 때는 **해당 대학의 공개 범위와 이용 조건을 확인**하고, 출처를 밝혀 주세요.
